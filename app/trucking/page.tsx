...
const PAIN_POINTS = [
  { icon: '📉', heading: 'Overpaying on renewal',           body: "Most carriers haven't compared rates in 2+ years. The average small fleet overpays by $2,000–$6,000 annually simply by staying with the same carrier." },
  { icon: '⚠️', heading: 'Conditional safety rating',       body: "A Conditional FMCSA rating can trigger premium spikes or non-renewal at your next policy date — often without warning. You need options before that happens." },
  { icon: '🚨', heading: "Coverage gaps you don't know about", body: "Primary liability is the minimum. Most carriers are missing cargo coverage at full value, bobtail protection, and business interruption — until a claim makes it obvious." },
  { icon: '📋', heading: 'New authority, no history',        body: "New carriers with no safety rating often get quoted the highest rates by default. There are carriers that specialise in new authority — most agents won't find them for you." },
]
...
const FAQS = [
  { q: 'How long does a quote comparison take?',                  a: "About 3 minutes. You provide basic information about your operation — DOT number, fleet size, cargo type — and receive quotes from multiple carriers that specialise in commercial trucking." },
  { q: 'Do I have to talk to an agent?',                          a: "No. The comparison process is online. If you want to speak with someone before binding, that option is available — but it's not required to get your quotes." },
  { q: 'Will this affect my current policy?',                     a: "No. Getting quotes is not a commitment. You can compare and decide whether switching makes financial sense without touching your current coverage." },
  { q: 'I have a Conditional safety rating. Can I still get quotes?', a: "Yes — and this is exactly the right time to shop. Several carriers in the comparison specialise in Conditional-rated fleets and offer competitive rates with risk management programs that can help improve your score." },
  { q: "What's the federal liability minimum?",                   a: "For general freight, $750K. For household goods, $300K. For hazmat, $1M–$5M depending on commodity. Many shippers and brokers now require $1M regardless of federal minimums." },
]
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div>
              ...
              <div className="flex flex-wrap gap-4 animate-fade-up delay-400">
                <Link href="/go/insurance" className="btn-amber px-8 py-4 rounded-lg font-display tracking-widest text-ink text-lg">
                  GET FREE QUOTES →
                </Link>
              </div>
              <div className="grid grid-cols-3 gap-4 mt-10 pt-8 border-t border-ghost/20 animate-fade-up delay-500">
                {[{ v: '3 min', l: 'to compare' }, { v: '$0', l: 'cost to you' }, { v: '10+', l: 'carriers compared' }].map(s => (
                  <div key={s.l}>
            ...
            <div className="bg-slate rounded-2xl border border-ghost/30 p-8 animate-fade-up delay-300">
              <div className="font-mono text-amber text-xs tracking-widest uppercase mb-6">What the comparison covers</div>
              <div className="space-y-4">
                {COVERAGE_TYPES.map(c => (
                  <div key={c.name} className="flex items-start gap-3">
                    <div className={`w-5 h-5 rounded flex-shrink-0 flex items-center justify-center mt-0.5 ${c.required ? 'bg-amber/20 border border-amber/40' : 'bg-ghost/15 border border-ghost/25'}`}>
                      {c.required ? <span className="text-amber text-xs font-bold">✓</span> : <span className="text-ghost text-xs">+</span>}
                    </div>
                    <div>
              ...
              <Link href="/go/insurance" className="mt-8 w-full btn-amber px-4 py-3 rounded-lg font-display tracking-widest text-ink text-sm flex items-center justify-center gap-2">
                SEE MY RATES →
              </Link>
            </div>
          </div>
      ...
      <section className="bg-navy border-y border-ghost/20 py-20 px-6">
        <div className="max-w-7xl mx-auto">
          <div className="mb-12">
            ...
            <h2 className="font-display text-ice text-4xl md:text-6xl leading-none">DOES ANY OF<br/>THIS SOUND FAMILIAR?</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {PAIN_POINTS.map((p, i) => (
              <div key={p.heading} className={`rounded-xl border border-ghost/25 bg-slate p-8 animate-fade-up delay-${(i+1)*100}`}>
          ...
          <div className="mt-8 text-center">
            <Link href="/go/insurance" className="btn-amber inline-flex px-10 py-4 rounded-lg font-display tracking-widest text-ink text-lg">
      ...
      <section className="max-w-7xl mx-auto px-6 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            <Link key={tool.href} href={tool.href} className="card-hover group rounded-xl border border-ghost/25 bg-slate p-6">
      ...
      <section className="max-w-7xl mx-auto px-6 pb-16">
        <div className="rounded-2xl border border-amber/20 bg-amber/5 p-10 relative overflow-hidden">
          ...
          <div className="relative max-w-2xl">
            ...
            <h3 className="font-display text-ice text-4xl md:text-5xl leading-none mb-4">WHAT HAPPENS IF<br/>YOUR TRUCKS STOP?</h3>
            <p className="text-mist leading-relaxed mb-6">
              ...
              But <span className="text-ice font-medium">nothing covers the income you lose while you&apos;re down</span> — unless you have business interruption coverage.
            </p>
            <p className="text-mist leading-relaxed mb-6">
              For a 5-truck fleet, a 60-day shutdown is <span className="text-amber font-medium">$40,000+ in lost revenue</span>.
              Most carriers don&apos;t carry this coverage because nobody explained it to them at renewal. It&apos;s included in the comparison.
            </p>
            <Link href="/go/insurance" className="btn-amber inline-flex px-8 py-4 rounded-lg font-display tracking-widest text-ink">
